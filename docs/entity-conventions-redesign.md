...
- Field references are values, not spelled strings: the build step emits `<Model>Fields`, a constant mirroring the model whose leaves are the field paths (`SalesOrderFields.lines.item.type`), so specifications read `where(so.customerId, '=', id)` with completion at every level. The type behind it: `FieldPath<T>` is every property and dotted relation path of the model, a builder that declared a join or raw selection widens what it accepts to those aliases, and `raw()` covers the rest. The operators and the value narrow with the property's type as well (`OperatorFor`, `ParamFor`, `ConditionValue`): text takes equality, patterns, and membership; numbers add order comparisons and `BETWEEN`; dates take comparisons and `BETWEEN` with `Date` values; a checkbox takes equality with a boolean or `'T'`/`'F'`. The builder binds what NetSuite compares: a `Date` becomes `TO_DATE(?, ...)` with the calendar date (and the time for a datetime field), a boolean on a checkbox field becomes `'T'`/`'F'`. `useText` switches to the text operators and string values, since the comparison is against `BUILTIN.DF`. Aliases and `raw()` references have no known type and accept everything.
