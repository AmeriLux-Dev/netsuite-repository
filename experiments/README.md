...
| `bisect-root-query.js` | The open-orders root query exactly as the 1.0.0 runtime builds it, without lines: the OR group on `status`, a sort on a column object outside `query.columns` versus the selected column, a DISPLAY column aliased with its own field id, the whole root query, and the separate line query with its sort in both forms. | Run 2026-09-09: everything renders except a sort whose column object is not in `query.columns`, which fails with a bare `UNEXPECTED_ERROR` on both roots. The OR group, the DISPLAY alias, and both whole queries are fine. |
...
- **A sort must reference one of `query.columns`.** `createSort` with a column object created only for the sort renders `UNEXPECTED_ERROR` with no message. The compiler sorts on the selected column and appends a hidden `__sort<n>` column when the sorted field is not selected.
